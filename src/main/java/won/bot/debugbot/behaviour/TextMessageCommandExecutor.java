...
public class TextMessageCommandExecutor extends BaseEventBotAction {
    ...
    public TextMessageCommandExecutor(EventListenerContext eventListenerContext, List<BotCommand> botCommands) {
                ...
                try {
                    if (message != null) {
                        ...
                        for (BotCommand botCommand : botCommands) {
                            if (botCommand.matchesCommand(message)) {
                                try {
                                    botCommand.execute(con, botCommand.getMatcher(message));
                                    break;
                                } catch (UnsupportedOperationException e) {
                                    logger.warn("BotCommand cant be executed due to: {}", e.getMessage());
                                }
                            }
                        }
                    }
